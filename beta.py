...
def register(): 
    global screen1, fullname, email, password, repassword, gender, tnc, company_id, company_name, company_city,security_q,security_a # making all entry field variable global 
    fullname, email, password, repassword, tnc, company_id, company_name, company_city,security_q,security_a = StringVar(),StringVar(),StringVar(),StringVar(),IntVar(),StringVar(),StringVar(),StringVar(),StringVar(),StringVar() 
    screen1 = Toplevel(screen)                
    screen1.title("Registeration") 
    adjustWindow(screen1) # configuring the window 
    image2,submit_button,proceed_button= PhotoImage(file="bg_reg.png"),PhotoImage(file="submit_button_3.png"),PhotoImage(file="proceed_to_login_3.png")
    label_for_image= Label(screen1, image=image2)
    label_for_image.pack()#lace(x=0, y=0, relwidth=1, relheight=1)
    label_text,labelx,labely = ["Full Name ","E-mail ID ","Password ","Re-Password ","Company Info : ","Security Question :"],[.343,.342,.3428,.360,.3,.3],[.180,.27,.355,.445,.56,.645]
    ...
    ttk.Entry(screen1, textvar=fullname , width='39').place(relx=.5, rely=.215, anchor="center")
    ttk.Entry(screen1, textvar=email , width='39').place(relx=.5, rely=.305, anchor="center")
    ttk.Entry(screen1, textvar=password, show="*" , width='39').place(relx=.5, rely=.39, anchor="center")
    ttk.Entry(screen1, textvar=repassword, show="*" , width='39').place(relx=.5, rely=.48, anchor="center")
    ...
    company_id.set('ID') 
    droplist.place(relx=.465, rely=.56, anchor="center")
    droplist = ttk.OptionMenu(screen1, company_name, *name_list) 
    droplist.config(width=6) 
    company_name.set('Name')
    droplist.place(relx=.6, rely=.56, anchor="center")
    droplist = ttk.OptionMenu(screen1, company_city, *city_list) 
    droplist.config(width=5) 
    company_city.set('City')
    droplist.place(relx=.75, rely=.56, anchor="center")
    security_questions = ['Security Questions',"What was your first car?","What is your mother's maiden name?","What is the name of your first pet?","What elementary school did you attend?","What is the name of the town where you were born?"]
    droplist = ttk.OptionMenu(screen1, security_q, *security_questions) 
    droplist.config(width= 35) 
    security_q.set('Questions')
    droplist.place(relx=.6, rely=.65, anchor="center")
    ttk.Entry(screen1, textvar=security_a , width='40').place(relx=.6, rely=.7, anchor="center")
    ttk.Checkbutton(screen1, text="I accept all terms and conditions ", variable=tnc).place(relx=.5, rely=.77, anchor="center")
    Button(screen1, image = submit_button , border = 0,command=register_user).place(relx=.5, rely=.84, anchor="center")
    Button(screen1, image = proceed_button , border = 0, command=screen1.destroy).place(relx=.5, rely=.92, anchor="center")
    screen1.mainloop()

def register_user(): 
    
    if fullname.get() and email.get() and password.get() and repassword.get()  and company_id.get() and company_name.get() and company_city.get() and security_q.get() and security_a.get(): # checking for all empty values in entry field 
                    ...
                    if password.get() == repassword.get(): # checking both password match or not 
                        if all(x.isalpha() or x.isspace() for x in fullname.get()) and (len(fullname.get()) > 0):
                            ...
                            if company_info:
                                ...
                                connection = pymysql.connect(host="localhost", user="root", passwd="", database="sales") # database connection 
                                cursor = connection.cursor() 
                                insert_query = "INSERT INTO company_users (fullname, email, password, company_id, company_name, company_city, security_question,answer) VALUES('"+ fullname.get() + "', '"+ email.get() + "', '"+ password.get() + "', '"+ company_id.get() + "', '"+ company_name.get() + "', '"+ company_city.get() +"', '"+ security_q.get() +"', '"+ security_a.get() + "' );" # queries for inserting values 
...
def forgot_password():
    global screen1_1,email_id,name,ans
    global submit_button
    submit_button = PhotoImage(file="submit_button_3.png")
    email_id,name,ans= StringVar(),StringVar(),StringVar()#,StringVar(),StringVar(),StringVar(),IntVar(),StringVar(),StringVar(),StringVar() 
    screen1_1 = Toplevel(screen)                
    screen1_1.title("Forgot Password") 
    adjustWindow(screen1_1) # configuring the window 
    image2,submit_button,proceed_button= PhotoImage(file="forgot.png"),PhotoImage(file="submit_button_3.png"),PhotoImage(file="proceed_to_login_3.png")
    label_for_image= Label(screen1_1, image=image2)
    label_for_image.pack()#lace(x=0, y=0, relwidth=1, relheight=1)
    Label(screen1_1, text="E-mail ID ", font=("Roboto lt", 8 , 'italic'), bg='black', fg='white').place(relx=.337, rely=.28, anchor="center")
    Label(screen1_1, text="Name ", font=("Roboto lt", 8, 'italic'), bg='black', fg='white').place(relx=.325, rely=.36, anchor="center")
    ttk.Entry(screen1_1, textvar=email_id , width='39').place(relx=.5, rely=.32, anchor="center")
    ttk.Entry(screen1_1, textvar=name , width='39').place(relx=.5, rely=.40, anchor="center")
    Button(screen1_1, image = submit_button , border = 0,command=fetch_user).place(relx=.5, rely=.465, anchor="center")
    Button(screen1_1, image = proceed_button , border = 0, command=screen1_1.destroy).place(relx=.5, rely=.885, anchor="center")
    screen1_1.mainloop()
    
def fetch_user():
    global ans
    ans = StringVar()
    if email_id.get() and name.get():
        connection = pymysql.connect(host="localhost", user="root", passwd="", database="sales") # database connection 
        cursor = connection.cursor() 
        select_query =  "SELECT * FROM company_users where email = '" + email_id.get() + "' AND fullname = '" + name.get() + "';" # queries for retrieving values 
        cursor.execute(select_query) # executing the queries 
        user_info = cursor.fetchall() 
        if user_info:
            Label(screen1_1, text="Question : " + str(user_info[0][-2]),width='48', font=("Roboto", 10 ), bg='black', fg='white').place(relx=.5, rely=.525,anchor='center')
            Label(screen1_1, text="Answer ", font=("Roboto lt", 8, 'italic'), bg='black', fg='white').place(relx=.332, rely=.575, anchor="center")
            ttk.Entry(screen1_1, textvar=ans, width='39').place(relx=.5, rely=.615, anchor="center")
            Button(screen1_1, image = submit_button , border = 0,command=fetch_ans).place(relx=.5, rely=.68, anchor="center")
        else:
            Label(screen1_1, text="⚠  Email-id and Name not found in database ", fg="red",font=("Roboto", 11), width='3000', anchor=W, bg='black').place(x=0, y=570) 
            return 
        connection.commit() # commiting the connection then closing it. 
        connection.close()
    else: 
        Label(screen1_1, text="⚠  Please fill all the details ", fg="red",font=("Roboto", 11), width='3000', anchor=W, bg='black').place(x=0, y=570) 
        return 
...
def fetch_ans():
    if ans.get():
        ...
        cursor.execute(select_query) # executing the queries 
        user_info = cursor.fetchall() 
        if user_info[0][-1]==ans.get() :
            Label(screen1_1, text="Your Password : " + str(user_info[0][3]),width='25', font=("Roboto", 11 ), bg='black', fg='white').place(relx=.5, rely=.75,anchor='center')
            Label(screen1_1, text=" Success ", fg="green",font=("Roboto", 11), width='3000', anchor=W, bg='white').place(x=0, y=570) 
        else:
            Label(screen1_1, text="⚠  Answer does not match ", fg="red",font=("Roboto", 11), width='3000', anchor=W, bg='black').place(x=0, y=570) 
            return 
        connection.commit() # commiting the connection then closing it. 
        connection.close()
    else: 
        Label(screen1_1, text="⚠  Please fill the details ", fg="red",font=("Roboto", 11), width='3000', anchor=W, bg='black').place(x=0, y=570) 
        return 
...
def main_screen(): 
    ...
    image1, login_button, register_button,forgot_button = PhotoImage(file="login_final_1.png"),PhotoImage(file="login_button_3.png"),PhotoImage(file="register_button_3.png"),PhotoImage(file="forgot_button_thin.png")
    ...
    Button(screen, image = forgot_button , border = 0, command=forgot_password).place(relx=.59, rely=.535, anchor="center")
    screen.mainloop() 
